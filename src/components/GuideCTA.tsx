...
export default function GuideCTA() {
  ...
  const handleStartShopping = async () => {
    ...
    setNavigating(true);
    try {
      await fetch("/api/auth/seen-guide", { method: "POST", credentials: "include" });
    ...
    <section className="bg-gray-900 text-white rounded-xl p-8 text-center">
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-center">
        <button
          ...
          disabled={navigating}
          className="inline-flex items-center justify-center gap-2 bg-primary-500 text-white font-semibold px-8 py-3 rounded-lg hover:bg-primary-600 transition-colors disabled:opacity-50"
        >
          {navigating ? "Loading..." : "Start Shopping"}
        </button>
        <a
