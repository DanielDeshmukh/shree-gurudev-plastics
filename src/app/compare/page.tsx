    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center px-4">
          <h1 className="text-2xl md:text-4xl font-bold text-gray-900 mb-4">No Products to Compare</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 md:py-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Comparing {compareCount} product{compareCount !== 1 ? "s" : ""}</p>
          </div>
                    ...
                    <td key={item.id} className="py-4 px-5 border-r border-gray-100 last:border-r-0">
                      {attr.key === "name" && (
                        <div className="flex flex-col items-center gap-2">
                          <button onClick={() => removeCompare(item.id)} className="text-xs text-red-500 hover:text-red-600 underline">Remove</button>
              ...
              <div className="flex gap-3 p-4">
                <div className="w-20 h-20 shrink-0 rounded-lg overflow-hidden border border-gray-200">
                  {item.imageUrl ? (
                    ...
                    <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-400 text-xs">No</div>
                  )}
                </div>
                ...
                <button onClick={() => removeCompare(item.id)} className="text-xs text-red-500 hover:text-red-600 self-start">Remove</button>
              </div>
              <div className="grid grid-cols-2 gap-px bg-gray-200 border-t border-gray-200">
                <div className="bg-gray-50 px-3 py-2"><span className="text-[10px] text-gray-500 block">Color</span><span className="text-xs text-gray-900">{item.color || "—"}</span></div>
                <div className="bg-white px-3 py-2"><span className="text-[10px] text-gray-500 block">Size</span><span className="text-xs text-gray-900">{item.size || "—"}</span></div>
                <div className="bg-gray-50 px-3 py-2"><span className="text-[10px] text-gray-500 block">Category</span><span className="text-xs text-gray-900">{item.category || "—"}</span></div>
